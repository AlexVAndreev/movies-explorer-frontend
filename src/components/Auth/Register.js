...
function Register({ signUp, regError, setRegError }) {
  ...
  const { values, handleChange, validatorErrors, setIsValid } = Validations();
    ...
    <main className="auth">
      ...
      <form className="auth__form" onSubmit={handleSubmit}>
        ...
        <div className="auth__message">
          {validatorErrors &&
          Object.values(validatorErrors).filter((item) => item !== "").length >
            0
            ? Object.entries(validatorErrors).map((item, ind) => {
                ...
                return (
                  <p
                    key={ind}
                    className="auth__message-text auth__message-text-error"
                  >
                    {`${item[0]} ${item[1]}`}
                  </p>
                );
